...
if (!file.exists("data/SAFI_clean.csv")) {
  ...
  # Clean data
  df <- read.csv("data/SAFI_clean.csv", stringsAsFactors = FALSE)
...
if (!file.exists("data/interviews_plotting.csv")) {
  ...
  interviews_plotting <- df %>%
    ...
    mutate(
      memb_assoc = na_if(memb_assoc, "NULL"),
    ...
    mutate(
      ...
      number_items = if_else(items_owned == "no_listed_items", 0L, n())
    ) %>%
    pivot_wider(
